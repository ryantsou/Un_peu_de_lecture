...
#include <algorithm>
    ...
    const std::vector<Livre>& Bibliotheque::livres() const {
        return livres_; 
    }
    const std::vector<Lecteur>& Bibliotheque::lecteurs() const { 
        return lecteurs_; 
    }
    const std::vector<Emprunt>& Bibliotheque::emprunts() const { 
        return emprunts_; 
    }
    const std::vector<Auteur>& Bibliotheque::auteurs() const { 
        return auteurs_; 
    }
